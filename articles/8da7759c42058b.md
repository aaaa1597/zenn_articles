---
title: "React+TypeScript+R3Fのtutorial応用編7(FBXモデルでアニメーション)"
emoji: "👋"
type: "tech"
topics:
  - "react"
...
# Abstract
FBXモデルの表示とアニメーションしてみた。
[前回](https://zenn.dev/rg687076/articles/581c5a4715fa40)は表示だけだったので、今回はアニメーションデータも読込んで動かすのを目標に。
使ったモデルはMixiamoってサイトのモデルをありがたく使わせてもらいました。→ [コレ](https://www.mixamo.com/#/?page=1&type=Character)

:::message
↓表示できなかったFBXモデルも存在してて。
いつか絶対リベンジする。
...
# 結論
今回の成果物はココ↓
~~https://github.com/aaaa1597/react-r3f-advanced007~~
↑gh-pagesを使って上げたら、デバッグでは動かなくなってしまった。
大した差分ないから、react-r3f-advanced006から作って下さい。
...
- 前回コードから始める。[react-r3f-advanced006](https://github.com/aaaa1597/react-r3f-advanced006)
...
$ ren react-r3f-advanced006 react-r3f-advanced007
$ cd react-r3f-advanced007
```
...
```
# 準備2
1. モデル一式は[前回](https://zenn.dev/rg687076/articles/581c5a4715fa40)同様。
2. モーションデータも[Mixamo](https://www.mixamo.com/#/?page=1&type=Character)からダウンロードする。
...
Without Skinを選ぶとモーションデータだけになる。
                           ↓
そのまま、DLしたふぃあるををプロジェクトフォルダの"react-r3f-advanced007/public/assets"配下へ。

## ちなみにモーションデータとモデルデータの区別は。
...
※[Autodesk FBX Review 1.5.3.0](https://www.fileeagle.com/software/2645/Autodesk-FBX-Review)については、ここからダウンロードしてね。
